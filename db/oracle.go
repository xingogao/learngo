...
func main() {
	...
	src := "SELECT c.id, c.content FROM zemr_emr_content c WHERE c.title = '住院患者入院护理评估记录单'"  /*rownum < 2 AND*/
	...
	stmtAdd, err := db.Prepare("insert into NEO_NURSING_ASSESSMENT (nid, emr_id, " +
		"pname, sex, age, dept, bed, event, " +
		"ryzd, xyzd, lxr, " +
		"ryfs, yw, sw, qt, jws, " +
		"tw, mb, hx, xy, tz, " +
		"sz, qxzt, ms, mx, " +
...
func resolveContent(row map[string]string) (data map[string]string, err error) {
	...
	doc.Find("td.UnderLine").Each(func(i int, s *goquery.Selection) {
		...
		if text == "/" {
			...
			data[label] = enc.ConvertString(text)
		}
	})
